...
from app.huey.amelia_api_async import AmeliaApiAsync
...
from app.huey.helpers import handle_response_of_tech_passports, run_async_task
from app.huey.amelia_api_async import APIGrids, APIRoutes
...
async def sync_buildings():
    ...
    response = await amelia_api.get(APIRoutes.BUILDINGS_WITH_QUERY, params=params)
    if response is None:
        msg = "Buildings response is none"
        logger.error(msg)
        return msg

    response_data = ReturnTypeFromJsonQuery[BuildingPostSchema](**response.json())
...
async def sync_floors():
    ...
    response = await amelia_api.get(APIRoutes.FLOORS_WITH_QUERY, params=params)
    if response is None:
...
async def sync_rooms(delay: float=config.API_CALLS_DELAY, building_id: int | None = None):
    ...
    response = await amelia_api.get(APIRoutes.ROOMS_WITH_QUERY, params=params)
    if response is None:
...
async def sync_tech_passports(delay: float = config.API_CALLS_DELAY, facility_id: int = 1, building_ids: list[int] | None = None, offset: int = 0):
    ...
    try:
        ...
        for i in range(start, ids_len):
            room_id = rooms_ids[i]
            response = await amelia_api.get(APIRoutes.TECH_PASSPORT_WITH_ID + str(room_id))
            sleep(delay)
            if response is None:
                continue
